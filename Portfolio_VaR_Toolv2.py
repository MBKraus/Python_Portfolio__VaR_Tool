...
class PageTwo(wx.Panel):
    ...
    def myListener(self, arg1, arg2, arg3, arg4):
        ...
        portfolio.rename(columns={'returns': 'Portfolio',}, inplace=True)
        ...
        axes_5.hist(portfolio["Portfolio"].dropna(), bins=50, normed=True, histtype='stepfilled', alpha=0.5)
        ...
        portfolio_cum = (1 + portfolio["Portfolio"].dropna()).cumprod()

        axes_A.plot(ret_index.index, ret_index[stocks[0]])
        axes_A.xaxis.set_major_locator(years)
        axes_A.xaxis.set_major_formatter(yearsFmt)

        axes_B.plot(ret_index.index, ret_index[stocks[1]])
        axes_B.xaxis.set_major_locator(years)
        axes_B.xaxis.set_major_formatter(yearsFmt)

        axes_C.plot(ret_index.index, ret_index[stocks[2]])
        axes_C.xaxis.set_major_locator(years)
        axes_C.xaxis.set_major_formatter(yearsFmt)

        axes_D.plot(ret_index.index, ret_index[stocks[3]])
...
class PageThree(wx.Panel):
    ...
    def myListener(self, arg1, arg2, arg3, arg4):
        ...
        portfolio_hist_ret = portfolio["Portfolio"].dropna()
        ...
        portfolio_return_daily = portfolio["Portfolio"].dropna().mean()
        portfolio_std = portfolio["Portfolio"].dropna().std()
        ...
        for item in range(len(stocks)):
            ...
            S = stock_data[stocks[item]].iloc[-1]
            T = 252
